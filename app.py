...
class PDFProposta(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 10, f"Página {self.page_no()} | Gerado em {datetime.date.today().strftime('%d/%m/%Y')}", 0, 0, 'C')
...
def gerar_documento_proposta(dados_cliente, total, itens_proposta):
    ...
    pdf.cell(0, 10, f"PREPARADO PARA: {dados_cliente['nome'].upper()}", ln=True)
    pdf.set_font("Arial", '', 11)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 7, f"Segmento: {dados_cliente['segmento']}", ln=True)
    pdf.cell(0, 7, f"Data de Emissão: {datetime.date.today().strftime('%d/%m/%Y')}", ln=True)
    
    # Escopo e Investimento
    pdf.ln(10)
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(0, 10, "1. INVESTIMENTO E HONORÁRIOS", ln=True)
    pdf.set_font("Arial", '', 11)
    pdf.multi_cell(0, 7, "Com base nas informações fornecidas e no diagnóstico do perfil operacional da empresa, definimos os seguintes honorários mensais:")
    
    # Tabela de Preço
    pdf.ln(5)
    pdf.set_fill_color(245, 245, 245)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(140, 12, " Descrição do Serviço", 1, 0, 'L', True)
    pdf.cell(50, 12, " Valor Mensal", 1, 1, 'C', True)
    
    pdf.set_font("Arial", '', 11)
    pdf.cell(140, 12, " Assessoria Contábil, Fiscal e Trabalhista Especializada", 1)
    pdf.set_font("Arial", 'B', 11)
    ...
    pdf.ln(20)
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "2. FORMALIZAÇÃO E ACEITE", ln=True)
    pdf.set_font("Arial", '', 11)
    pdf.multi_cell(0, 7, "Esta proposta é válida por 10 dias. Para formalização, utilize o portal GOV.BR para assinatura digital e realize o upload do arquivo assinado em nosso sistema.")
    ...
    pdf.cell(0, 5, "Espaço reservado para Assinatura Digital (GOV.BR)", 0, 1, 'C')
...
# --- MÓDULO: CONFIGURAÇÕES (Mantido e Ajustado) ---
